...
for d = 1:LD
    ...
    for g = 1:Lg
        ...
        % h_mars = h_earth
        for i = hselect
            Rest(g,d) = D50(d)*ust(g,i)/v;                                %[-] Particle Reynolds number (vR84,Leeuw20,nino03), Reynolds shear velocity number (Klein05), Boundary Reynolds number (Miedema10), fluid reynolds number (Bagnold1966)
            ...
            AdvL(g,d) = u(g,i)*h(i)/ws(g,d);                              %[m] Advection length
        end
    end
end
...
for g = 1:Lg
    for d = 1:LD
        ...
        Qb_E42(g,d) = 2.1*exp(-0.391/shield(g,d));                          %[-] Einstein 1942 as in Carrillo 2021
        ...
        Qb_W66(g,d) = 12*(max(shield(g,d)-shield_cr(g,d),0))^1.5;           %[-] Wilson 1966 as in Fernandez Luque and van Beek 1976
        qb_FB(g,d) = Qb_FB(g,d)*(grav(g)*R*D50(d)^3)^0.5;                   %[m3/ms]
        Qb_FB(g,d) = 5.7*(max(shield(g,d)-shield_cr(g,d),0))^1.5;           %[-] Fernandez Luque and van Beek 1976 (also in de Leeuw 2020)
        qb_FB(g,d) = Qb_FB(g,d)*(grav(g)*R*D50(d)^3)^0.5;                   %[m3/ms]
        ...
        Qb_WP1(g,d) = 4.93*(max(shield(g,d)-0.047,0))^1.6;                  %[-] Wong and Parker 2006, s=2.55 | theta_cr=0.047
        Qb_WP2(g,d) = 3.97*(max(shield(g,d)-0.0495,0))^1.5;                 %[-] Wong and Parker 2006, s=2.55 | theta_cr=0.0495
        ...
        Qb_C(g,d) = 13*shield(g,d)^1.5*exp(-0.05/shield(g,d)^1.5);          %[-] Cheng 2002 (also in Carrillo 2021), 2.53<s<2.69, 0.73<S<1.2%, 0.068<D<1.27ft, 0.093<Q<1.118ft3/s | theta_cr=0.05
        qb_C(g,d) = Qb_C(g,d)*(grav(g)*R*D50(d)^3)^0.5;                     %[m3/ms]
        
        %A theta^0.5*(theta-theta_cr)
        Qb_N(g,d) = 12*shield(g,d)^0.5*(max(shield(g,d)-shield_cr(g,d),0)); %[-] Nielsen 1992 as in Soulsby 2005 and Camenen 2005 (different in Carrillo 2021) (payed book download)
...
for g=1:Lg
    for d=1:LD
        ...
        Ca_ML(g,d) = 0.065*0.003*S0(g,d)/(1+0.004*S0(g,d));
        ...
        qs_IK(g,d) = integral(fun,a_IK(g),h(g));                            %Itakura and Kishi 1980
        fun = @(z) Ca_CR(g,d) * ( ((h(g)-z)./z) * (a_CR(g)/(h(g)-a_CR(g))) ).^Rouse(g,d);
        qs_CR(g,d) = integral(fun,a_CR(g),h(g));                            %Celik and Rodi 1984
        fun = @(z) Ca_AF(g,d) * ( ((h(g)-z)./z) * (a_AF(g)/(h(g)-a_AF(g))) ).^Rouse(g,d);
        qs_AF(g,d) = integral(fun,a_AF(g),h(g));                            %Akiyama and Fukushima 1986
...
for a = 1
    ...
    leg = legend(s1(1),['Mars h=' num2str(h(hselect)) ' m^3/s'],['Earth h=' num2str(h(hselect)) ' m^3/s'],['Unsuitable predictor']);
    set(leg,'location','northeast');
    leg2 = legend(s1(2),['Mars h=' num2str(h(hselect)) ' m^3/s'],['Earth h=' num2str(h(hselect)) ' m^3/s'],['Unsuitable predictor']);
    ...
    print(f1,'-dpng',[output '/FlowOnMars4_Transport_Dh'],'-r400');
    print(f1,'-dpdf',[output '/FlowOnMars4_Transport_Dh'],'-r400');    
end