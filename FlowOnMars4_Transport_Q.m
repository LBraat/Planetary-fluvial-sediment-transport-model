...
for d = 1:LD
    for i = 1:LQ
        for g = 1:Lg
            h(g,i,d) = h_guess+1;
            while abs(h_guess-h(g,i,d))>0.0001
                h_guess = h(g,i,d);
                Rw(g,i,d) = (h_guess*W)/(h_guess+h_guess+W); %[m] Hydraulic radius
                f(g,i,d) = 8/(5.75*log10(12*h_guess/ks(d)))^2; %[-] friction factor
                u(g,i,d) = (8*grav(g)*Rw(g,i,d)*S/f(g,i,d))^0.5; %[m/s] Velocity
                h(g,i,d) = Q(i)/(W*u(g,i,d)); %[m] Water depth
            end
            tau(g,i,d) = rho*grav(g)*Rw(g,i,d)*S; %[N/m2] Bed shear stress
            ust(g,i,d) = (tau(g,i,d)/rho)^0.5; %[m/s] Shear velocity
            Fr(g,i,d) = u(g,i,d)/(grav(g)*h(g,i,d))^0.5; %[-] Froude number
            Re(g,i,d) = u(g,i,d)*h(g,i,d)/v; %[-] Reynolds number
        end
    end
end
...
for d = 1:LD
    for g = 1:Lg
        ...
        % Q_mars = Q_earth
        for i = 1:LQ
            Rest(g,i,d) = D(d)*ust(g,i,d)/v; %[-] Particle Reynolds number (vR84,Leeuw20), Reynolds shear velocity number (Klein05), Boundary Reynolds number (Miedema10)
            shield(g,i,d) = tau(g,i,d)/((rhos-rho)*grav(g)*D(d)); %[-] Shields parameter/Particle mobility parameter
            lambda(g,i,d) = ust(g,i,d)/ws(g,d); %[-] Movability number (Liu 1958)
            AdvL(g,i,d) = u(g,i,d)*h(g,i,d)/ws(g,d); %[m] Advection length
        end
    end
end
clear C1 C2
...
for i = Qselect
    for g = 1:Lg
        for d = 1:LD
            ...
            T0(g,d) = max(((ust(g,i,d)^2-ust_cr(g,d)^2)/ust_cr(g,d)^2),0);

            %A(theta-theta_cr)^B
            Qb_MPM(g,d) = 8*(max(shield(g,i,d)-shield_cr(g,d),0))^(3/2); %[-] Meyer-Peter and Muller 1948 (also in D3D and Carrillo 2021) | theta_cr=0.047
            qb_MPM(g,d) = Qb_MPM(g,d)*(grav(g)*R*D(d)^3)^0.5; %[m3/ms]
            Qb_MP(g,d) = (4*max(shield(g,i,d)-0.188,0))^(3/2); %[-] Meyer-Peter 1949/1951 as in Carrillo 2021, 1.25<s<4.2
            ...
            Qb_S(g,d) = 4.2*S^0.6*(u(g,i,d)/ust(g,i,d))*shield(g,i,d)^0.5*(max(shield(g,i,d)-shield_cr(g,d),0)); %[-] Smart 1984
            ...
            %no theta_cr
            Qb_EH(g,d) = 0.1*shield(g,i,d)^(5/2) / (f(g,i,d)/4); %[-] Engelund and Hansen as in D3D
            ...
            G2(g,d) = rhos*D(d)^2*ust(g,i,d)^2/(rho*14*v^2); %[?] Grain flow number (Bagnold 1966), 150<G2<6000:tan(phi)=-0.236, G2<150:tan(phi)=0.7, G2>6000:tan(phi)=0.374
            ...
            if G2(g,d)>150 && G2(g,d)<6000
                qb_B(g,d) = eb(g,d)*u(g,i,d)*tau(g,i,d)/((rhos-rho)*grav(g)*cos(S*(-0.236-tan(S)))); %[m3/ms]
            elseif G2(g,d)<=150
                qb_B(g,d) = eb(g,d)*u(g,i,d)*tau(g,i,d)/((rhos-rho)*grav(g)*cos(S*(0.7-tan(S))));
            elseif G2(g,d)>=6000
                qb_B(g,d) = eb(g,d)*u(g,i,d)*tau(g,i,d)/((rhos-rho)*grav(g)*cos(S*(0.374-tan(S))));
            else
                qb_B(g,d) = NaN;
            end
...
for i = Qselect
    for g=1:Lg
        for d=1:LD
            ...
            a_IK(g,d) = 0.05*h(g,i,d); %Itakura and Kishi 1980 as in Garcia and Parker 1991
            a_CR(g,d) = 0.05*h(g,i,d); %Celik and Rodi 1984 as in Garcia and Parker 1991
            a_AF(g,d) = 0.05*h(g,i,d); %Akiyama and Fukushima 1986 as in Garcia and Parker 1991
            a_GP(g,d) = 0.05*h(g,i,d); %Garcia and Parker 1991
            a_WP(g,d) = 0.05*h(g,i,d); %Wright and Parker 2004
            a_dL(g,d) = 0.1*h(g,i,d); %de Leeuw 2020
            a_vR(g,d) = max(0.01*squeeze(h(g,i,d)),ks(d)); %van Rijn 1984
            a_SM(g,d) = 26.3*(max(shield(g,i,d)-shield_cr(g,d),0))*D(d)+ks(d); %Smith and McLean 1977
            a_ML(g,d) = 0.68*(tau(g,i,d)/tau_cr(g,d))*D(d)/(1+(0.0204*log(D(d)*100)^2+0.022*log(D(d)*100)+0.0709)*(tau(g,i,d)/tau_cr(g,d))); %McLean 1992

            %Rouse number
            Rouse(g,d) = ws(g,d)/(Kappa*ust(g,i,d)); %Rouse 1937
            ...
            Ca_vR(g,d) = 0.015*(D(d)/a_vR(g,d))*S0(g,d)^1.5/Dst(g,d)^0.3; %van Rijn 1984
            Z_AF(g,d) = Rep(g,d)^0.5*ust(g,i,d)/ws(g,d);
            if Z_AF(g,d)<5
                Ca_AF(g,d) = 0;
            ...
            Z_GP(g,d) = Rep(g,d)^0.6*ust(g,i,d)/ws(g,d);
            Ca_GP(g,d) = 0.00000013*Z_GP(g,d)^5/(1+(0.00000013/0.3)*Z_GP(g,d)^5); %Garcia and Parker 1991
            Z_WP(g,d) = Rep(g,d)^0.6*ust(g,i,d)/ws(g,d);
            Ca_WP(g,d) = 0.00000078*Z_WP(g,d)^5/(1+(0.00000078/0.3)*Z_WP(g,d)^5); %Wright and Parker 2004
            %Ca_dL(g,d) = 0.000474*(ust(g,i,d)/ws(g,d))^1.77*Fr(g,i,d)^1.18; %de Leeuw 2020, equation 25
            Ca_dL(g,d) = 4.74*10^-4*((ust(g,i,d)/ws(g,d))^1.5*Fr(g,i,d)-0.015)^1.18 / (1+3*(4.74*10^-4*((ust(g,i,d)/ws(g,d))^1.5*Fr(g,i,d)-0.015)^1.18)); %de Leeuw 2020, equation 26b
            fun = @(eta) (((1-eta)./eta).*(0.05/(1-0.05))).^Rouse(g,d);
            I_CR(g,d) = integral(fun,0.05,1);
            Cm_CR(g,d) = 0.034*(1-(ks(d)/h(g,i,d))^0.06)*ust(g,i,d)^2*u(g,i,d)/(grav(g)*R*h(g,i,d)*ws(g,d));
            ...
            Ca_IK(g,d) = max(0.008*(0.14*ust(g,i,d)*Omega_IK(g,d)/(ws(g,d)*shield(g,i,d))-1),0); %Itakura and Kishi 1980 as in Garcia and Parker 1991, goes negative without max(CA_IK,0)...

            %             %Profile C and u
            %             zz{g,d} = 0:0.01:h(g,i,d); %[m] height, vector over water depth for Rouse profile
            %             Lz = length(zz{g,d});
            %             for j=1:Lz
            %                 Cz_E50{g,d}(j) = Ca_E50(g,d) * ( ((h(g,i,d)-zz{g,d}(j))/zz{g,d}(j)) * (a_E50(d)/(h(g,i,d)-a_E50(d))) )^Rouse(g,d);
            %                 %Cz_EF{g,d}(j) = Ca_EF(g,d) * ( ((h(g,i,d)-zz{g,d}(j))/zz{g,d}(j)) * (a_EF(d)/(h(g,i,d)-a_EF(d))) )^Rouse(g,d);
            %                 %                 Cz_IK(g,d,j) = Ca_IK(g,d) * ( ((h(g,i,d)-zz(g,j))/zz(g,j)) * (a_IK(g,d)/(h(g,i,d)-a_IK(g,d))) )^Rouse(g,d);
            %                 %                 Cz_CR(g,d,j) = Ca_CR(g,d) * ( ((h(g,i,d)-zz(g,j))/zz(g,j)) * (a_CR(g,d)/(h(g,i,d)-a_CR(g,d))) )^Rouse(g,d);
            %                 %                 Cz_AF(g,d,j) = Ca_AF(g,d) * ( ((h(g,i,d)-zz(g,j))/zz(g,j)) * (a_AF(g,d)/(h(g,i,d)-a_AF(g,d))) )^Rouse(g,d);
            %                 %                 Cz_GP(g,d,j) = Ca_GP(g,d) * ( ((h(g,i,d)-zz(g,j))/zz(g,j)) * (a_GP(g,d)/(h(g,i,d)-a_GP(g,d))) )^Rouse(g,d);
            %                 %                 Cz_WP(g,d,j) = Ca_WP(g,d) * ( ((h(g,i,d)-zz(g,j))/zz(g,j)) * (a_WP(g,d)/(h(g,i,d)-a_WP(g,d))) )^Rouse(g,d);
            %                 %                 Cz_dL(g,d,j) = Ca_dL(g,d) * ( ((h(g,i,d)-zz(g,j))/zz(g,j)) * (a_dL(g,d)/(h(g,i,d)-a_dL(g,d))) )^Rouse(g,d);
            %                 %                 Cz_vR(g,d,j) = Ca_vR(g,d) * ( ((h(g,i,d)-zz(g,j))/zz(g,j)) * (a_vR(g,d)/(h(g,i,d)-a_vR(g,d))) )^Rouse(g,d);
            %                 %                 Cz_SM(g,d,j) = Ca_SM(g,d) * ( ((h(g,i,d)-zz(g,j))/zz(g,j)) * (a_SM(g,d)/(h(g,i,d)-a_SM(g,d))) )^Rouse(g,d);
            %                 %                 Cz_ML(g,d,j) = Ca_ML(g,d) * ( ((h(g,i,d)-zz(g,j))/zz(g,j)) * (a_ML(g,d)/(h(g,i,d)-a_ML(g,d))) )^Rouse(g,d);
            %                 uz{g,d}(j) = (ust(g,i,d) / Kappa) * log(zz{g,d}(j)/0.033);
            %             end

            %Integral
            fun = @(z) (ust(g,i,d) * Ca_dL(g,d) / Kappa) * ( ((h(g,i,d)-z)./z) * (a_dL(g,d)/(h(g,i,d)-a_dL(g,d))) ).^Rouse(g,d) .* log(z/(0.033*ks(d)));
            qs_dL(g,d) = integral(fun,a_dL(g,d),h(g,i,d)); %de Leeuw 2020
            fun = @(z) (ust(g,i,d) * Ca_E50(g,d) / Kappa) * ( ((h(g,i,d)-z)./z) * (a_E50(d)/(h(g,i,d)-a_E50(d))) ).^Rouse(g,d) .* log(z/(0.033*ks(d)));
            qs_E50(g,d) = integral(fun,a_E50(d),h(g,i,d)); %Einstein 1950
            fun = @(z) (ust(g,i,d) * Ca_EF(g,d) / Kappa) * ( ((h(g,i,d)-z)./z) * (a_EF(d)/(h(g,i,d)-a_EF(d))) ).^Rouse(g,d) .* log(z/(0.033*ks(d)));
            qs_EF(g,d) = integral(fun,a_EF(d),h(g,i,d)); %Engelund and Fredsoe 1976
            fun = @(z) (ust(g,i,d) * Ca_IK(g,d) / Kappa) * ( ((h(g,i,d)-z)./z) * (a_IK(g,d)/(h(g,i,d)-a_IK(g,d))) ).^Rouse(g,d) .* log(z/(0.033*ks(d)));
            qs_IK(g,d) = integral(fun,a_IK(g,d),h(g,i,d)); %Itakura and Kishi 1980 as in Garcia and Parker 1991
            fun = @(z) (ust(g,i,d) * Ca_CR(g,d) / Kappa) * ( ((h(g,i,d)-z)./z) * (a_CR(g,d)/(h(g,i,d)-a_CR(g,d))) ).^Rouse(g,d) .* log(z/(0.033*ks(d)));
            qs_CR(g,d) = integral(fun,a_CR(g,d),h(g,i,d)); %Celik and Rodi 1984 as in Garcia and Parker 1991
            fun = @(z) (ust(g,i,d) * Ca_AF(g,d) / Kappa) * ( ((h(g,i,d)-z)./z) * (a_AF(g,d)/(h(g,i,d)-a_AF(g,d))) ).^Rouse(g,d) .* log(z/(0.033*ks(d)));
            qs_AF(g,d) = integral(fun,a_AF(g,d),h(g,i,d)); %Akiyama and Fukushima 1986 as in Garcia and Parker 1991
            fun = @(z) (ust(g,i,d) * Ca_GP(g,d) / Kappa) * ( ((h(g,i,d)-z)./z) * (a_GP(g,d)/(h(g,i,d)-a_GP(g,d))) ).^Rouse(g,d) .* log(z/(0.033*ks(d)));
            qs_GP(g,d) = integral(fun,a_GP(g,d),h(g,i,d)); %Garcia and Parker 1991
            fun = @(z) (ust(g,i,d) * Ca_WP(g,d) / Kappa) * ( ((h(g,i,d)-z)./z) * (a_WP(g,d)/(h(g,i,d)-a_WP(g,d))) ).^Rouse(g,d) .* log(z/(0.033*ks(d)));
            qs_WP(g,d) = integral(fun,a_WP(g,d),h(g,i,d)); %Wright and Parker 2004
            fun = @(z) (ust(g,i,d) * Ca_vR(g,d) / Kappa) * ( ((h(g,i,d)-z)./z) * (a_vR(g,d)/(h(g,i,d)-a_vR(g,d))) ).^Rouse(g,d) .* log(z/(0.033*ks(d)));
            qs_vR(g,d) = integral(fun,a_vR(g,d),h(g,i,d)); %van Rijn 1984
            fun = @(z) (ust(g,i,d) * Ca_SM(g,d) / Kappa) * ( ((h(g,i,d)-z)./z) * (a_SM(g,d)/(h(g,i,d)-a_SM(g,d))) ).^Rouse(g,d) .* log(z/(0.033*ks(d)));
            qs_SM(g,d) = integral(fun,a_SM(g,d),h(g,i,d)); %Smith and McLean 1977
            fun = @(z) (ust(g,i,d) * Ca_ML(g,d) / Kappa) * ( ((h(g,i,d)-z)./z) * (a_ML(g,d)/(h(g,i,d)-a_ML(g,d))) ).^Rouse(g,d) .* log(z/(0.033*ks(d)));
            qs_ML(g,d) = integral(fun,a_ML(g,d),h(g,i,d)); %McLean 1992

            % de Leeuw et al 2020 for total transport calculations
            ub(g,d) = 0.6 * u(g,i,d); %de Leeuw et al 2020 eq 20
            hb(g,d) = min(0.6 * (Fr(g,i,d) * (D(d)/h(g,i,d))^2)^0.3 * h(g,i,d), h(g,i,d)); %de Leeuw et al 2020 eq 21
            Cb(g,d) = qb(g,d,8) / (hb(g,d) * ub(g,d)); %de Leeuw et al 2020 eq 17
            fun = @(z) (ust(g,i,d) * Cb(g,d) / Kappa) * ( ((h(g,i,d)-z)./z) * (hb(g,d)/(h(g,i,d)-hb(g,d))) ).^Rouse(g,d) .* log(z/(0.033*ks(d)));
            qs_dL2(g,d) = integral(fun,hb(g,d),h(g,i,d)); %integrating between Hb and h is the same as integrating between Hb-a and a-h and adding those. So this way of writing is simpler.
        end
    end
end
...
for a = 0
    ...
    %leg1 = legend(s1(1),['Mars Q = ' num2str(Q(Qselect)) ' m^3/s'],['Earth Q = ' num2str(Q(Qselect)) ' m^3/s'],'Mars - used for total','Earth - used for total','unsuitable','location','northeast');
    %leg2 = legend(s1(2),['Mars Q = ' num2str(Q(Qselect)) ' m^3/s'],['Earth Q = ' num2str(Q(Qselect)) ' m^3/s'],'Mars - used for total','Earth - used for total','unsuitable','location','northeast');
    leg1 = legend(s1(1),['Mars Q = ' num2str(Q(Qselect)) ' m^3/s'],['Earth Q = ' num2str(Q(Qselect)) ' m^3/s'],'Mars - used for total','Earth - used for total','location','northeast');
    leg2 = legend(s1(2),['Mars Q = ' num2str(Q(Qselect)) ' m^3/s'],['Earth Q = ' num2str(Q(Qselect)) ' m^3/s'],'Mars - used for total','Earth - used for total','location','northeast');
...
for a = 1
    ...
    pl.vertical = 4; %number of vertical subplots
    pl.horizantal = 2; %number of horizontal subplots
    pl.lmarge = 0.09; %left margin
    pl.rmarge = 0.025; %right margin
    pl.bmarge = 0.055; %bottom margin
    pl.tmarge = 0.01; %top margin
    pl.intv = 0.05; %vertical space between graphs
    pl.inth = 0.09; %horizontal space between graphs
    pl.wt = (1 - pl.lmarge - pl.rmarge - ((pl.horizantal-1)*pl.inth)) / pl.horizantal;
    pl.ht = (1 - pl.bmarge - pl.tmarge - ((pl.vertical-1)*pl.intv)) / pl.vertical;
    y = 0.95; %placing subplot letter, y
    D50L = D(1);
...
for a = 1
    ...
    f1 = figure('units','centimeters','PaperUnits','centimeters', ...
        ...
        'PaperSize',[pl.width pl.height],'visible','off');
    ...
    set(gcf,'renderer','painters');
    print(f1,'-dpng',[output '/FlowOnMars4_Transport_poster'],'-r400');
    print(f1,'-dpdf',[output '/FlowOnMars4_Transport_poster'],'-r400');
